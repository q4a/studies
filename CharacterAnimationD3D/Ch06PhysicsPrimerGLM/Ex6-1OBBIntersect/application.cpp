...
HRESULT Application::Init(HINSTANCE hInstance, bool windowed) {
    ...
    RegisterClass(&wc);
    m_mainWindow = ::CreateWindow("D3DWND", "Character Animation with Direct3D: Example 6.1", WS_OVERLAPPEDWINDOW, 0, 0, rc.right - rc.left, rc.bottom - rc.top, 0, 0, hInstance, 0);
    SetCursor(NULL);
...
void Application::Update(float deltaTime) {
    try {
        ...
        //Update Rotations
        if (KeyDown(VK_SPACE) && m_press <= 0.0f) {
            m_rot1 = glm::quat(rand()%1000 - 500.0f, rand()%1000 - 500.0f, rand()%1000 - 500.0f, rand()%1000 - 500.0f);
            m_rot2 = glm::quat(rand()%1000 - 500.0f, rand()%1000 - 500.0f, rand()%1000 - 500.0f, rand()%1000 - 500.0f);
            D3DXQuaternionNormalize(&m_rot1, &m_rot1);
            D3DXQuaternionNormalize(&m_rot2, &m_rot2);
            m_press = 0.3f;
        }
        m_press = max(m_press - deltaTime, 0.0f);

        //Update OBB's rotations
        ...
        try {
            ...
            if (SUCCEEDED(g_pDevice->BeginScene())) {
                ...
                if (m_pOBB1->Intersect(*m_pOBB2)) {
                    RECT rc = {10, 10, 0, 0};
                    g_pFont->DrawText(NULL, "Intersecting!", -1, &rc, DT_LEFT | DT_TOP | DT_NOCLIP, 0xff000000);
                }

                RECT rc = {10, 590, 0, 590};
                g_pFont->DrawText(NULL, "Press Space to rotate the boxes.", -1, &rc, DT_LEFT | DT_BOTTOM | DT_NOCLIP, 0x88000000);
