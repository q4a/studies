...
// the color like is should, and then it discretizes (离散化) the color into
// one of four colors.
float4 CelPixelShader(VertexToPixel input) : COLOR0
{
    ...
    if (intensity > 0.95)
        ...
        color = float4(0.7, 0.7, 0.7, 1) * color;
    else if (intensity > 0.05)
        color = float4(0.35, 0.35, 0.35, 1) * color;
    else
        color = float4(0.1, 0.1, 0.1, 1) * color;

    // toon shader 有很多种方法, 比如: color = round(intensity * 5) / 5 * color;
...
technique Toon
{
    // 注意: 先画 Outline (比模型稍大的黑背景), 再画 Cel 着色覆盖到黑背景上, 留下的便是外轮廓
    ...
    pass Pass1
    {
        ...
        // For our outline shader, we are actually telling it to ignore the front facing triangles and only draw the backwards ones.
        // Without this, the outline shader would draw the expanded model, and it would block out the rest of the model. All we would see is a big blob of black.
        // But since we are ignoring triangles that are facing the screen during the outline pass, nothing will be drawn there, and we will be able to see the second pass.
        CullMode = CW;  // back face: clockwise
    }
    ...
    pass Pass2
    {
        ...
        CullMode = CCW; // front face: counterclockwise
    }
}
