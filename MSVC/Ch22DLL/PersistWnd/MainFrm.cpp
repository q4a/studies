...
BEGIN_MESSAGE_MAP(CMainFrame, CPersistFrame)
    ON_WM_CREATE()
    ON_COMMAND(ID_TEST_COMPUTESQRT, &CMainFrame::OnTestComputesqrt)
    ON_COMMAND(ID_TEST_RYG, &CMainFrame::OnTestRyg)
END_MESSAGE_MAP()
...
{
    if( !CPersistFrame::PreCreateWindow(cs) )
        return FALSE;
...
void CMainFrame::OnTestComputesqrt()
{
    CComputeSqrtDlg dlg;
    dlg.DoModal();
}

void CMainFrame::OnTestRyg()
{
    CRygDlg dlg;
    dlg.DoModal();
}
