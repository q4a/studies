...
const LPCTSTR GU::APPTITLE = _T("Anti-Virus Game");
const int GU::SCREENW = 1024;
const int GU::SCREENH = 768;
...
double g_ScrollX = 0;
double g_ScrollY = 0;
LPDIRECT3DSURFACE9 g_Background = NULL;
const double VIRTUAL_LEVEL_SIZE = BUFFERW * 5;
double g_VirtualScrollX = 0;
...
PLAYER_STATES g_PlayerState = NORMAL;
PLAYER_STATES g_PlayerStatePrev = NORMAL;
...
BOOL GU::Game_Init(HWND window) {
    Direct3D_Init(window, SCREENW, SCREENH, FALSE);
    DirectInput_Init(window);
    DirectSound_Init(window);

    // create a font
    g_Font = MakeFont(_T("Arial Bold"), 24);
    g_DebugFont = MakeFont(_T("Arial"), 14);

    // load background
    LPDIRECT3DSURFACE9 image = NULL;
    image = LoadSurface(GetResPath(_T("binary.bmp")));
    if (!image) return FALSE;

    // load player sprite
    g_PlayerShip = LoadTexture(GetResPath(_T("ufo.png")));
    g_Player.x = 100;
    g_Player.y = 350;
    ...
    // load bullets
    g_PurpleFire = LoadTexture(GetResPath(_T("purplefire.tga")));
    ...
    // load enemy viruses
    g_Virus1Image = LoadTexture(GetResPath(_T("virus1.tga")));
    for (int n = 0; n < VIRUSES; n++) {
        g_Viruses[n].alive = TRUE;
        g_Viruses[n].width = 96;
        g_Viruses[n].height = 96;
        g_Viruses[n].x = (float) (1000 + rand() % BUFFERW);
        g_Viruses[n].y = (float) (rand() % SCREENH);
        g_Viruses[n].velx = (float) ((rand() % 8) * -1);
        g_Viruses[n].vely = (float) (rand() % 2 - 1);
    }

    // load gui elements
    g_EnergySlice = LoadTexture(GetResPath(_T("energyslice.tga")));
    g_HealthSlice = LoadTexture(GetResPath(_T("healthslice.tga")));

    // load audio files
    g_Tisk = LoadSound(GetResPath(_T("clip.wav")));
    g_Foom = LoadSound(GetResPath(_T("longfoom.wav")));
    g_Charging = LoadSound(GetResPath(_T("charging.wav")));

    // load memory fragments (energy)
    g_FragmentImage = LoadTexture(GetResPath(_T("fragment.tga")));
    for (int n = 0; n < FRAGMENTS; n++) {
        ...
        g_Fragments[n].x = (float) (rand() % BUFFERW);
...
template <class T>
String static ToString(const T & t, int places = 2) {
    basic_ostringstream<_TCHAR> oss;
...
void GU::Game_Run(HWND window) {
    ...
    if (g_CurrentTime > g_Refresh + 14) {
        ...
        if (!g_Paused) {
            ...
            g_ScrollX += 1.0;
            if (g_ScrollY < 0)
                g_ScrollY = BUFFERH - SCREENH;
            if (g_ScrollY > BUFFERH - SCREENH)
                g_ScrollY = 0;
            if (g_ScrollX < 0)
                g_ScrollX = BUFFERW - SCREENW;
            if (g_ScrollX > BUFFERW - SCREENW)
                g_ScrollX = 0;

            // update virtual scroll position
            g_VirtualScrollX += 1.0;
            if (g_VirtualScrollX > VIRTUAL_LEVEL_SIZE)
                g_VirtualScrollX = 0.0;

            // update overloaded bullets
            if (g_PlayerState == NORMAL && g_PlayerStatePrev == OVERLOADING) {
                int bulletcount = 0;
            ...
            for (int n = 0; n < BULLETS; n++) {
                if (g_Bullets[n].alive) {
                    ...
                    if (g_Bullets[n].x < 0 || g_Bullets[n].x > SCREENW || g_Bullets[n].y < 0 || g_Bullets[n].y > SCREENH)
                        g_Bullets[n].alive = FALSE;
                }
            }
            ...
            for (int n = 0; n < VIRUSES; n++) {
                if (g_Viruses[n].alive) {
                    ...
                    if (g_Viruses[n].y < -96.0f)
                        g_Viruses[n].y = SCREENH;
                    if (g_Viruses[n].y > SCREENH)
                        g_Viruses[n].y = -96.0f;
                }
            }
            ...
            for (int n = 0; n < FRAGMENTS; n++) {
                if (g_Fragments[n].alive) {
                    ...
                    if (g_Fragments[n].x < 0.0 - g_Fragments[n].width)
                        g_Fragments[n].x = BUFFERW;
                    if (g_Fragments[n].x > VIRTUAL_LEVEL_SIZE)
                        g_Fragments[n].x = 0.0;
                    if (g_Fragments[n].y < 0.0 - g_Fragments[n].height)
                        g_Fragments[n].y = SCREENH;
                    if (g_Fragments[n].y > SCREENH)
                        g_Fragments[n].y = 0.0;

                    if (g_PlayerState == CHARGING) {
    ...
    if (g_D3DDev->BeginScene()) {
        ...
        for (int n = 0; n < VIRUSES; n++) {
            ...
            if (g_Viruses[n].alive) {
                ...
                if (g_Viruses[n].x > -96.0f && g_Viruses[n].x < SCREENW)
        ...
        FontPrint(g_Font, 900, 0, _T("SCORE"), color);
        ...
        for (int n = 0; n < g_Health * 5; n++)
            Sprite_Transform_Draw(g_HealthSlice, 10 + n * 2, SCREENH - 21, 1, 20, 0, 1, 0.0f, 1.0f, 1.0f, healthcolor);

        // draw debug messages
        FontPrint(g_DebugFont, 0, y, _T(""), debugcolor);
        FontPrint(g_DebugFont, 0, y - 12, _T("Core FPS = ") + ToString(g_CoreFPS) + _T(" (") + ToString(1000.0 / g_CoreFPS) + _T(" ms)"), debugcolor);
        FontPrint(g_DebugFont, 0, y - 24, _T("Screen FPS = ") + ToString(g_ScreenFPS), debugcolor);
        FontPrint(g_DebugFont, 0, y - 36, _T("Ship X, Y = ") + ToString(g_Player.x) + _T(", ") + ToString(g_Player.y), debugcolor);
        FontPrint(g_DebugFont, 0, y - 48, _T("Bullets = ") + ToString(g_BulletCount));
        FontPrint(g_DebugFont, 0, y - 60, _T("Buffer Scroll = ") + ToString(g_ScrollX), debugcolor);
        FontPrint(g_DebugFont, 0, y - 72, _T("Virtual Scroll = ") + ToString(g_VirtualScrollX) + _T(" / ") + ToString(VIRTUAL_LEVEL_SIZE));
        FontPrint(g_DebugFont, 0, y - 84, _T("Fragment[0] = ") + ToString(g_Fragments[0].x) + _T(", ") + ToString(g_Fragments[0].y));
