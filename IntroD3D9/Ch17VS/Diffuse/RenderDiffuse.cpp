...
LPCTSTR RenderDiffuse::MeshDumpFile = _T("MeshDump.txt");
LPCTSTR RenderDiffuse::VertexShaderFile = _T("diffuse.txt");
...
void RenderDiffuse::DumpMesh(LPCTSTR fname, ID3DXMesh* mesh) {
    MeshDump dump;
    if (!dump.Init(fname))
        return;
